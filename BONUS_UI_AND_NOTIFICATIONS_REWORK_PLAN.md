...
- Доступ:
  ...
  - обычному капперу НЕ давать загрузку, но показать locked-состояние: “Обложка профиля доступна VIP-капперам”.
  - рядом показать CTA “Стать VIP” / “Подключить VIP” со ссылкой на `reverse("cabinet:vip_purchase")`.
- Endpoint:
...
- Template:
  ...
  - обычному капперу показать disabled/locked-кнопку “Сменить обложку” или compact-card:
    - title: `Обложка профиля`;
    - text: `Добавьте персональную обложку после подключения VIP.`;
    - link: `Стать VIP`;
    - без file input, чтобы пользователь не мог выбрать файл.
  - обычным пользователям, которые не капперы, блок не показывать.
- JS:
  - jQuery/AJAX или существующий стиль проекта;
  - без skeleton;
  - disable кнопки во время upload;
  - после успеха заменить background/image.
  - JS должен работать только если есть активный upload form/input; locked CTA не должен запускать upload logic.
