...
''')
    if old not in text:
        raise SystemExit("WebsiteSettings field marker not found")
...
''')
    if old not in text:
        raise SystemExit("WebsiteSettingsAdmin marker not found")
    admin.write_text(text.replace(old, new, 1), encoding="utf-8")


migration = Path("back/migrations/0005_website_settings_bookmaker_placements.py")
...
component = Path("templates/front/includes/_bookmaker_link.html")
component.write_text(dedent('''\
    {% if bookmaker %}
    <a
    ...
    new = dedent('''\
                    ...
                    <div class="match-detail-bookmaker-strip" aria-label="Коэффициенты и букмекеры">
                        ...
                        <div class="match-detail-featured-odds" aria-label="Основные коэффициенты 1 X 2">
                            <span class="match-detail-featured-odd">
                                <small>1</small>
                                <strong>{% if match.coupon_odds.home %}{{ match.coupon_odds.home|floatformat:2 }}{% else %}—{% endif %}</strong>
                            </span>
                            <span class="match-detail-featured-odd">
                                <small>X</small>
                                <strong>{% if match.coupon_odds.draw %}{{ match.coupon_odds.draw|floatformat:2 }}{% else %}—{% endif %}</strong>
                            </span>
                            <span class="match-detail-featured-odd">
                                <small>2</small>
                                <strong>{% if match.coupon_odds.away %}{{ match.coupon_odds.away|floatformat:2 }}{% else %}—{% endif %}</strong>
                            </span>
                        </div>
...
if 'class="prediction-bookmaker-links is-table"' not in text:
    old = dedent('''\
                        <div class="prediction-table-actions">
                            <button
''')
    new = dedent('''\
                        <div class="prediction-table-actions">
...
if marker not in text:
    block = dedent('''\
        ...
        @media (max-width: 760px) {
            ...
            .prediction-bookmaker-links .bookmaker-ref:nth-child(2) {
                display: none;
            }
        }
''')
